...
export class HomepageComponent implements OnInit {
      ...
      if (data == null) {
        this.customerService.getByEmail(email).subscribe(data => {
          ...
          this.favoriteService.post(new Favorites(0, new Customer(this.customer.userId), new Product(id))).subscribe(data => {
            this.toastr.success('Thêm sản phẩm yêu thích thành công!', 'Hệ thống');
  ...
  addCart(productId: number, price: number) {
    ...
    this.cartService.getCart(email).subscribe(data => {
      ...
      this.cartDetail = new CartDetail(0, 1, price, new Product(productId), new Cart(this.cart.cartId), this.size);
